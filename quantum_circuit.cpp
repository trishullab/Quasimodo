...
#include "cflobdd/CFLOBDD/wvector_fb_mul.h"
...
WeightedCFLOBDDQuantumCircuit::WeightedCFLOBDDQuantumCircuit(unsigned int numQubits, int seed) : QuantumCircuit(numQubits, seed)
{
    ...
    stateVector = WeightedVectorFloatBoostMul::MkBasisVector(level, s);
}
...
{
    numQubits = num;
    stateVector = WeightedVectorFloatBoostMul::MkBasisVector(numQubits, 0);
}

WEIGHTED_CFLOBDD_FLOAT_BOOST_MUL ApplyGateF(unsigned int n, unsigned int i, WEIGHTED_CFLOBDD_FLOAT_BOOST_MUL(*f)(unsigned int))
{
    if (n == 1)
    {
        return f(1);
        ...
        if (i < n/2)
        {
            WEIGHTED_CFLOBDD_FLOAT_BOOST_MUL T = WeightedMatrix1234FloatBoostMul::MkIdRelationInterleaved(level + 1);
            WEIGHTED_CFLOBDD_FLOAT_BOOST_MUL H = ApplyGateF(n/2, i, f);
            return WeightedMatrix1234FloatBoostMul::KroneckerProduct2Vocs(H, T);
        }
        else
        {
            WEIGHTED_CFLOBDD_FLOAT_BOOST_MUL T = WeightedMatrix1234FloatBoostMul::MkIdRelationInterleaved(level + 1);
            return WeightedMatrix1234FloatBoostMul::KroneckerProduct2Vocs(T, ApplyGateF(n/2, i - n/2, f)); 
        }
    }
}

WEIGHTED_CFLOBDD_FLOAT_BOOST_MUL ApplyGateFWithParam(unsigned int n, unsigned int i, WEIGHTED_CFLOBDD_FLOAT_BOOST_MUL(*f)(unsigned int, double), double theta)
{
    WEIGHTED_CFLOBDD_FLOAT_BOOST_MUL H = f(2, theta);
    if (i == 0)
    {
        WEIGHTED_CFLOBDD_FLOAT_BOOST_MUL I = WeightedMatrix1234FloatBoostMul::MkIdRelationInterleaved(2 * (n-1));
        return WeightedMatrix1234FloatBoostMul::KroneckerProduct2Vocs(H, I); 
    }
    else if (i == n - 1)
    {
        WEIGHTED_CFLOBDD_FLOAT_BOOST_MUL I = WeightedMatrix1234FloatBoostMul::MkIdRelationInterleaved(2 * (n-1));
        return WeightedMatrix1234FloatBoostMul::KroneckerProduct2Vocs(I, H);
    }
    else {
        WEIGHTED_CFLOBDD_FLOAT_BOOST_MUL I1 = WeightedMatrix1234FloatBoostMul::MkIdRelationInterleaved(2 * i);
        WEIGHTED_CFLOBDD_FLOAT_BOOST_MUL I2 = WeightedMatrix1234FloatBoostMul::MkIdRelationInterleaved(2 * (n - i - 1));
        auto T = WeightedMatrix1234FloatBoostMul::KroneckerProduct2Vocs(H, I2);
        return WeightedMatrix1234FloatBoostMul::KroneckerProduct2Vocs(I1, T);
    }
}
...
{
    auto H = ApplyGateF(std::pow(2, stateVector.root->level-1), index, WeightedMatrix1234FloatBoostMul::MkIdRelationInterleaved);
    stateVector = WeightedMatrix1234FloatBoostMul::MatrixMultiplyV4(H, stateVector);
}

void WeightedCFLOBDDQuantumCircuit::ApplyHadamardGate(unsigned int index)
{
    auto H = ApplyGateF(std::pow(2, stateVector.root->level-1), index, WeightedMatrix1234FloatBoostMul::MkWalshInterleaved);
    // H.print(std::cout);
    stateVector = WeightedMatrix1234FloatBoostMul::MatrixMultiplyV4(H, stateVector);
    // std::cout << std::endl;
    hadamard_count++;
...
void WeightedCFLOBDDQuantumCircuit::ApplyCNOTGate(long int controller, long int controlled)
{
    ...
    if (controller < controlled)
    {
        auto C = WeightedMatrix1234FloatBoostMul::MkCNOT(stateVector.root->level, std::pow(2, stateVector.root->level-1), controller, controlled);
        // stateVector.print(std::cout);
        stateVector = WeightedMatrix1234FloatBoostMul::MatrixMultiplyV4(C, stateVector);
        // stateVector.print(std::cout);
    }
}
...
std::string WeightedCFLOBDDQuantumCircuit::Measure() 
{
    ...
    tmp.ComputeWeightOfPathsAsAmpsToExits();
    std::uniform_real_distribution<double> dis(0.0, 1.0);
    return WeightedVectorFloatBoostMul::Sampling(tmp, true).substr(0, numQubits); 
}

void WeightedCFLOBDDQuantumCircuit::ApplyNOTGate(unsigned int index)
{
    auto X = ApplyGateF(std::pow(2, stateVector.root->level-1), index, WeightedMatrix1234FloatBoostMul::MkNegationMatrixInterleaved);
    stateVector = WeightedMatrix1234FloatBoostMul::MatrixMultiplyV4(X, stateVector);
}
